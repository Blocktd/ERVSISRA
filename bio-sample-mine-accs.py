...
def post_to_ncbi(ids, database, history):
    epost_query = Entrez.epost(db=database, id=",".join(ids), method="post", history=history)
...
def elink_from_history(webenv, query_key):
    elink_query = Entrez.elink(dbfrom="BioSample", db="sra", webenv=webenv, query_key=query_key, idtype="acc",
                               retmax=1000)
...
esearch_term = '\"' + species + '\"' + "[ORGANISM] AND public[filter] AND attribute disease[filter] "\
                               "AND biosample sra[filter]"
print esearch_term
esearch_query = Entrez.esearch(db="BioSample", term=esearch_term, retmax=100000, idtype="BioSampleId")
